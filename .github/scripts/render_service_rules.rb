...
def load_render_config(render_file)
...
def database_resource_names(database)
  return [] unless database.is_a?(Hash)

  names = [database["name"].to_s.strip]
  replica_names = Array(database["readReplicas"]).map do |replica|
    replica.is_a?(Hash) ? replica["name"].to_s.strip : ""
  end

  (names + replica_names).reject(&:empty?).uniq
end

def service_uses_database?(yaml, service_name, database_name)
  Array(yaml["projects"]).any? do |project|
    next false unless project.is_a?(Hash)

    Array(project["environments"]).any? do |environment|
      next false unless environment.is_a?(Hash)

      Array(environment["services"]).any? do |service|
        next false unless service.is_a?(Hash)
        next false unless service["name"].to_s.strip == service_name

        Array(service["envVars"]).any? do |env_var|
          env_var.is_a?(Hash) && env_var.dig("fromDatabase", "name").to_s.strip == database_name
        end
      end
    end
  end
...
def lifecycle_excluded_service_names(yaml)
  return [] if service_uses_database?(yaml, LIFECYCLE_DATABASE_CONSUMER_SERVICE, CONDITIONAL_LIFECYCLE_DATABASE)
  ...
  Array(yaml["projects"]).flat_map do |project|
    ...
    Array(project["environments"]).flat_map do |environment|
      ...
      Array(environment["databases"]).flat_map do |database|
        next [] unless database.is_a?(Hash)
        next [] unless database["name"].to_s.strip == CONDITIONAL_LIFECYCLE_DATABASE
...
def apply_lifecycle_resource_policy(yaml, services)
  excluded_names = lifecycle_excluded_service_names(yaml)
  selected_services = services.reject { |service| excluded_names.include?(service) }

  {
    "selected_services" => selected_services,
    "excluded_services" => services.select { |service| excluded_names.include?(service) }.uniq,
  }
end
...
def load_available_services(render_file)
  ...
  services = Array(yaml["projects"]).flat_map do |project|
    ...
    Array(project["environments"]).flat_map do |environment|
      ...
      service_names + database_names
    end
  end

  services = services.reject(&:empty?).uniq
  services = apply_lifecycle_resource_policy(yaml, services).fetch("selected_services")
  abort_with("No render resources were found in #{render_file}.") if services.empty?
...
def command_filter(render_file, input_file, output_file)
  abort_with("Usage: render_service_rules.rb filter RENDER_FILE INPUT_FILE OUTPUT_FILE") unless output_file

  yaml = load_render_config(render_file)
  services = JSON.parse(File.read(input_file))
  unless services.is_a?(Array)
    abort_with("Invalid services input at #{input_file}: expected a JSON array.")
  end

  normalized_services = services.map { |service| service.to_s.strip }.reject(&:empty?).uniq
  write_configure_output(output_file, apply_lifecycle_resource_policy(yaml, normalized_services))
rescue JSON::ParserError => e
  abort_with("Invalid JSON in #{input_file}: #{e.message}")
end
    ...
    write_configure_output(
      output_file,
      {
        ...
        "summary" => [
          ...
          "Targets: #{Array(existing_rule['services']).join(', ')}",
        ],
      },
    )
    return
    ...
    result = {
      ...
      "action" => action,
        ...
        "summary" => [
          ...
          "Targets: #{selected_services.join(', ')}",
          "Preview handling: selected web services plus active preview children",
        ],
      }
...
when "filter"
  command_filter(ARGV[0], ARGV[1], ARGV[2])
when "plan"
  command_plan(ARGV[0], ARGV[1])
when "record"
  command_record(ARGV[0], ARGV[1])
else
  abort_with("Usage: render_service_rules.rb [configure|filter|plan|record] ...")
end
