...
case "$ACTION" in
  suspend)
    ...
    postgres_target_state="suspended"
    endpoint="suspend"
    verb_label="Suspend"
    ;;
  resume)
    ...
    postgres_target_state="not_suspended"
    endpoint="resume"
    verb_label="Resume"
    ;;
  *)
...
requested_services_file="$tmp_dir/requested-services.json"
filtered_services_file="$tmp_dir/filtered-services.json"

printf '%s' "$SERVICES_JSON" > "$requested_services_file"

if ! ruby .github/scripts/render_service_rules.rb filter render.yaml "$requested_services_file" "$filtered_services_file"; then
  echo "::error::Failed to evaluate lifecycle targets from render.yaml."
  append_summary "${verb_label} failed: could not evaluate lifecycle targets from render.yaml."
  exit 1
fi

excluded_services_json="$(jq -c '.excluded_services // []' "$filtered_services_file")"
SERVICES_JSON="$(jq -c '.selected_services // []' "$filtered_services_file")"

if [ "$(jq -r 'length' <<<"$excluded_services_json")" -gt 0 ]; then
  excluded_services_csv="$(jq -r 'join(", ")' <<<"$excluded_services_json")"
  echo "::notice::Skipping lifecycle-excluded Render resources: ${excluded_services_csv}."
  append_summary "${verb_label} note: skipped ${excluded_services_csv} because gfed-medusa-be does not reference them in render.yaml envVars."
fi

if ! jq -e 'type == "array" and length > 0' >/dev/null 2>&1 <<<"$SERVICES_JSON"; then
  echo "::error::No Render resources remain after applying the render.yaml lifecycle policy."
  append_summary "${verb_label} failed: no Render resources remain after applying the render.yaml lifecycle policy."
  exit 1
fi
...
list_all_postgres() {
  ...
  while :; do
    ...
    local page_resp
    if ! page_resp="$(render_api_get "/v1/postgres" "${query_args[@]}")"; then
      echo "::error::Failed to list Render Postgres instances."
      return 1
    fi
    ...
    if [ "$page_status" -ge 300 ]; then
      echo "::error::Listing Render Postgres instances failed (HTTP ${page_status})."
      echo "$page_body"
      return 1
    fi
...
all_postgres_file="$tmp_dir/all-postgres.json"
...
postgres_targets_file="$tmp_dir/postgres-targets.json"
...
if ! list_all_postgres "$all_postgres_file"; then
  append_summary "${verb_label} failed: could not list Render Postgres instances."
  exit 1
fi
...
jq --argjson selected "$SERVICES_JSON" --arg postgres_target_state "$postgres_target_state" '
  [
    .[] | .postgres as $pg
    | select($pg != null)
    | select(($selected | index($pg.name)) != null)
    | {
        id: $pg.id,
        name: $pg.name,
        current_state: ($pg.suspended // (if ($pg.status // "") == "suspended" then "suspended" elif ($pg.status // "") == "" then "" else "not_suspended" end)),
        kind: "postgres",
        target_state: $postgres_target_state
      }
  ] | unique_by(.id)
' "$all_postgres_file" > "$postgres_targets_file"

base_names_json="$(jq -c 'map(.name)' "$base_targets_file")"
key_value_names_json="$(jq -c 'map(.name)' "$key_value_targets_file")"
postgres_names_json="$(jq -c 'map(.name)' "$postgres_targets_file")"
found_names_json="$(jq -cn --argjson base "$base_names_json" --argjson key_values "$key_value_names_json" --argjson postgres "$postgres_names_json" '
  ($base + $key_values + $postgres) | unique
')"
...
jq -s 'add | unique_by(.id)' "$base_targets_file" "$preview_targets_file" "$key_value_targets_file" "$postgres_targets_file" > "$targets_file"
...
postgres_count="$(jq -r 'length' "$postgres_targets_file")"
...
if [ "$key_value_count" -gt 0 ] || [ "$postgres_count" -gt 0 ]; then
  echo "::notice::Resolved ${target_count} Render target(s), including Postgres instances, Key Value instances, and active preview children when present."
  append_summary "${verb_label} target scope: selected services, selected databases, selected Key Value instances, plus active preview children (${target_count} Render target(s))."
else
...
while IFS= read -r target_b64; do
  ...
  case "$target_kind" in
    ...
    key_value)
      ...
      state_field="status"
      state_filter='.status // empty'
      ;;
    postgres)
      target_label="Render Postgres instance '${service_name}'"
      action_path="/v1/postgres/${service_id}/${endpoint}"
      detail_path="/v1/postgres/${service_id}"
      state_field="suspended"
      state_filter='.suspended // (if (.status // "") == "suspended" then "suspended" elif (.status // "") == "" then "" else "not_suspended" end)'
      ;;
    *)
  ...
  while [ "$attempt" -le "$max_attempts" ]; do
    if ! detail_resp="$(render_api_get "$detail_path")"; then
      ...
      if [ "$detail_status" -ge 300 ]; then
        ...
        polled_state="$(echo "$detail_body" | jq -r "$state_filter")"
